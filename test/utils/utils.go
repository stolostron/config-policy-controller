...
func GetMetrics(metricPatterns ...string) []string {
	podCmd := exec.CommandContext(testCtx, "kubectl", "get", "pod", "-n=open-cluster-management-agent-addon",
	...
	if propPodName == "No" || propPodName == "" {
		...
		cmd = exec.CommandContext(
			testCtx, "kubectl", "exec", "-n=open-cluster-management-agent-addon", propPodName, "-c",
